...
class SX127x(BaseModem):
    # Note: Currently the LoRa module defines SX1276, SX1277, SX1278 and SX1279
    # as different classes (set up in __init__.py) but they are actually all
    # this class. Perhaps in the future some subclasses with software enforced
    # limits can be added to this driver, but the differences appear very minor:
    #
    ...
    def __init__(self, spi, cs, dio0=None, dio1=None, reset=None, lora_cfg=None, ant_sw=None):
        ...
        if self._dio0:
            self._dio0.irq(self._radio_isr, trigger=umachine.Pin.IRQ_RISING)
            if self._dio1:
                self._dio1.irq(self._radio_isr, trigger=umachine.Pin.IRQ_RISING)
    ...
    def configure(self, lora_cfg):
        ...
        if "preamble_len" in lora_cfg:
            ...
            self._reg_write(_REG_PREAMBLE_LEN_MSB, ustruct.pack(">H", self._preamble_len))
    ...
    def _standby(self):
        ...
        if old_mode not in (_OPMODE_MODE_STDBY, _OPMODE_MODE_SLEEP):
            # If we just cancelled transmitting or receiving, clear any pending IRQs
            self._reg_write(_REG_IRQ_FLAGS, 0xFF)
    ...
    def is_idle(self):
        ...
        # next time poll_receive() or poll_transmit() is called.
    ...
    def start_receive(self, timeout_ms=None, continuous=False, rx_length=0xFF):
        ...
        super().start_receive(timeout_ms, continuous, rx_length)  # sets self._rx
    ...
    def prepare_transmit(self, packet):
        ...
        # (won't happen unless poll_transmit() was not called)
