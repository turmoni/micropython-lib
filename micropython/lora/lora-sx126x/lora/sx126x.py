...
class SX126x(BaseModem):
    ...
    #
    # Don't construct this directly, construct SX1261 or SX1262
    def __init__(
        self,
        spi,
        cs,
        ...
        if (status[0] != _STATUS_MODE_STANDBY_RC and status[0] != _STATUS_MODE_STANDBY_HSE32) or (
            ...
            raise RuntimeError("Read invalid status {}.".format(status))
        ...
        if dio1:
            ...
            dio1.irq(self._radio_isr, trigger=umachine.Pin.IRQ_RISING)
    ...
    def calibrate(self):
        ...
        utime.sleep_us(_CALIBRATE_TYPICAL_TIME_US)
    ...
    def _wait_not_busy(self, timeout_us):
        ...
        ticks_diff = 0
        while self._busy():
            ticks_diff = utime.ticks_diff(utime.ticks_us(), start)
    ...
    def _cmd(self, fmt, *write_args, n_read=0, write_buf=None, read_buf=None):
        ...
        # fmt - Format string suitable for use with ustruct.pack. First item should be 'B' and
        # corresponds to the command opcode.
        # write_args - Arguments suitable for ustruct.pack using fmt. First argument should be a
        # command opcode byte.
        #
        # Optional arguments:
        ...
        wrlen = ustruct.calcsize(fmt)
        assert n_read + wrlen <= len(self._buf)  # if this fails, make _buf bigger!
        ustruct.pack_into(fmt, self._buf, 0, *write_args)
